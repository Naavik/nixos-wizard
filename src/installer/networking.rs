...
use crate::{
    ...
    widget::{
        Button, CheckBox, ConfigWidget, HelpModal, InfoBox, LineEditor, StrList,
        WidgetBox,
    },
};
...
impl NetworkConfig {
    ...
    pub fn display_widget(installer: &mut Installer) -> Option<Box<dyn ConfigWidget>> {
        ...
        if let Some(ref backend) = installer.network_backend {
            lines.push(vec![(None, "Network backend: ")]);
            lines.push(vec![(HIGHLIGHT, backend.as_str())]);
        } else {
            lines.push(vec![(None, "Network backend: ")]);
            lines.push(vec![(HIGHLIGHT, "Not configured")]);
        }
        ...
        if let Some(ref ssh) = installer.ssh_config {
            if ssh.enable {
                lines.push(vec![(None, "SSH: ")]);
                lines.push(vec![(HIGHLIGHT, "Enabled")]);
                lines.push(vec![
                    (None, "Port: "),
                    (HIGHLIGHT, "22"), // Use static string for now
                ]);
            } else {
                lines.push(vec![(None, "SSH: ")]);
                lines.push(vec![(HIGHLIGHT, "Disabled")]);
            }
        } else {
            lines.push(vec![(None, "SSH: ")]);
            lines.push(vec![(HIGHLIGHT, "Not configured")]);
    ...
    pub fn page_info<'a>() -> (String, Vec<Line<'a>>) {
        (
            ...
            styled_block(vec![
                vec![(
                    None,
                    "Configure network settings for your system.",
                )],
                vec![(
                    None,
...
impl Page for NetworkConfig {
    fn render(&mut self, installer: &mut Installer, f: &mut Frame, area: Rect) {
        ...
        let mut info_lines = vec![
            vec![(HIGHLIGHT, "Current Network Configuration")],
            vec![(None, "")],
        ];
        
        if let Some(ref backend) = installer.network_backend {
            info_lines.push(vec![
                (None, "Network Backend: "),
                (HIGHLIGHT, backend.as_str()),
            ]);
        } else {
            info_lines.push(vec![
                (None, "Network Backend: "),
        ...
        if let Some(ref ssh) = installer.ssh_config {
            if ssh.enable {
                ...
                info_lines.push(vec![
                    (None, "  Password Auth: "),
                    (HIGHLIGHT, if ssh.password_auth { "Yes" } else { "No" }),
                ]);
                info_lines.push(vec![
                    (None, "  Root Login: "),
                    (HIGHLIGHT, if ssh.root_login { "Yes" } else { "No" }),
                ]);
            } else {
                info_lines.push(vec![
                    (None, "SSH Server: "),
...
impl SshConfig {
    ...
    fn save_to_config(&self, installer: &mut Installer) {
        let port = self.port_input.get_value()
