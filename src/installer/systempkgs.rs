...
impl SystemPackages {
  pub fn new(selected_pkgs: Vec<String>, available_pkgs: Vec<String>) -> Self {
    ...
    let mut available = OptimizedStrList::new(
      "Available Packages",
      package_manager.get_available_packages(),
    );
    available.focus();
    let selected =
      OptimizedStrList::new("Selected Packages", package_manager.get_selected_packages());
